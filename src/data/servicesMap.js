export const servicesData = [
    {
        id: 1,
        name: "Inteligencia de negocios",
        steps: [
            {
                stepId: 1,
                image: "https://media.istockphoto.com/id/1480239219/es/foto/un-analista-utiliza-una-computadora-y-un-panel-de-control-para-el-an%C3%A1lisis-de-negocios-de.jpg?s=612x612&w=0&k=20&c=BYWWoy2RordEovmXC5gWexCm529iXMgPZp6RoAXgpp8=",
                stepName: "Recopilación e Integración de Datos",
                stepDescription: "Recopilar y consolidar datos dispersos de múltiples fuentes en una única fuente."
            },
            {
                stepId: 2,
                image: "https://media.istockphoto.com/id/1402667894/es/foto/pantalla-de-conferencia-de-energ%C3%ADa.jpg?s=612x612&w=0&k=20&c=if8B0E4TJ1ohnU-yZKGXosQfx2ctOh7JTWllo6VxWkc=",
                stepName: "Análisis Profundo y Acción Inteligente",
                stepDescription: "Análisis descriptivos hasta predictivos y prescriptivos."
            },
            {
                stepId: 3,
                image: "https://images.pexels.com/photos/265087/pexels-photo-265087.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
                stepName: "Tableros",
                stepDescription: "Poderosas visualizaciones interactivas y tableros dinámicos."
            }
        ],
        target: "bi-home-article"
    },
    {
        id: 2,
        name: "Desarrollo Web",
        steps: [
            {
                stepId: 1,
                image: "https://media.istockphoto.com/id/1206638576/es/foto/dise%C3%B1ador-de-sitios-web-dise%C3%B1o-de-aplicaciones-de-planificaci%C3%B3n-creativa-desarrollo-de.jpg?s=612x612&w=0&k=20&c=7gJbKYiqy8hFfy8HAvgdjPCZTx0gCdsqppq1mnu_LUA=",
                stepName: "Diseño Web Creativo y Atractivo",
                stepDescription: "Sitios web impactantes y visualmente atractivos."
            },
            {
                stepId: 2,
                image: "https://media.istockphoto.com/id/1279128074/es/vector/personas-que-usan-el-cuadro-de-b%C3%BAsqueda-para-la-consulta.jpg?s=612x612&w=0&k=20&c=WIMz_-6aEh_T_iF2GvCpdsgW3A8TVdYSblcQvYsFfEE=",
                stepName: "Optimización para Motores de Búsqueda (SEO)",
                stepDescription: "Maximización de la visibilidad de tu sitio web en los motores de búsqueda."
            },
            {
                stepId: 3,
                image: "https://media.istockphoto.com/id/1169675135/es/foto/bangkok-thailand-26-junio-2019-manos-del-hombre-utilizan-iphone-7-plus-con-la-aplicaci%C3%B3n-de.jpg?s=612x612&w=0&k=20&c=VJfeMV5bxNpPnSI3mxQcYNDiDiCyEO7AczjPIOp_WYE=",
                stepName: "Integración de Redes Sociales y Marketing Digital",
                stepDescription: "Integración de tus perfiles de redes sociales y estrategias de marketing digital."
            }
        ],
        target: "web-home-article"
    },
    {
        id: 3,
        name: "Infraestructura IT",
        steps: [
            {
                stepId: 1,
                stepName: "Paso 1 IT",
                stepDescription: "Descripcion 1 IT"
            },
            {
                stepId: 2,
                stepName: "Paso 2 IT",
                stepDescription: "Descripcion 2 IT"
            },
            {
                stepId: 3,
                stepName: "Paso 3 IT",
                stepDescription: "Descripcion 3 IT"
            }
        ],
        target: "it-home-article"
    }
];